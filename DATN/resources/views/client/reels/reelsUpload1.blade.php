...
<script src="https://vjs.zencdn.net/7.14.3/video.min.js"></script>
            ...
            <div class="iq-card-body">
                <div class="row"  style="border-bottom: 1px solid var(--iq-border-light);">
                    <div class="col-8">
                        ...
                        <textarea id="description" cols="80" rows="5" placeholder="Chia sẻ thêm về Video của bạn tại đây..." style="border-radius: 10px; border: none; background: rgba(0, 0, 0, 0.13); padding: 10px;"></textarea>
                        
                        <div id="charCount" style="color: #00000075; margin-top: -30px; position: absolute; margin-left: 75%; font-size: 10px">0 ký tự/2000</div>
                        <p style="color: black">Ảnh bìa <svg data-toggle="tooltip" data-placement="top" title="Chọn hoặc tải ảnh bìa lên từ thiết bị của bạn. Ảnh bìa đẹp có thể thu hút sự quan tâm của người xem một cách hiệu quả."  fill="rgba(0,0,0,0.48)" height="1em" width="1em" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg"><path d="M24 6a18 18 0 1 0 0 36 18 18 0 0 0 0-36ZM2 24a22 22 0 1 1 44 0 22 22 0 0 1-44 0Zm25-8a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm-4 6a1 1 0 0 0-1 1v11a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1V23a1 1 0 0 0-1-1h-2Z"></path></svg></p>
                        <button id="selectImageButton" class="btn btn-sm btn-primary view-more">
                            Chọn ảnh
                        </button>
                        <input type="file" id="cover" accept="image/*" style="display: none;">
                        <div id="preview" style="margin-top: 10px;"></div>
                    </div>

                    {{-- <div class="col-4 text-center">
                        <p style="color: black">Xem trước</p>
                            <video id="videoPreview" width="100%" controls autoplay>
                                <source id="videoSource" src="{{ $videoURL }}" type="video/mp4" style="width: 100%; border: 15px;">
                            </video>
                    </div> --}}

                    <div class="col-4 text-center">
                        <p style="color: black">Xem trước</p>
                         <iframe id="videoFrame" width="100%" height="600" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                    </div>
                    
                </div>
                <div class="iq-card-body">
                    <button type="submit" style="width: 90px; height: 35px; border: none; color: white; background: #11e1c2; border-radius: 5px;">
                        Đăng
                    </button>
                    <button id="cancelButton" style="width: 90px; height: 35px; border: none; color: black; background-color: #0000000e; border-radius: 5px;">
                        Hủy bỏ
                    </button>
                    
                    <!-- Popup xác nhận -->
                    <div id="confirmPopup" style=";display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: white; border: 1px solid #ccc; border-radius: 5px; padding: 20px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); z-index: 1000;">
                        <p>Bạn có chắc chắn muốn hủy bỏ không?</p>
                        <div class="text-center">
                            <button id="yesButton"  type="submit" style="width: 90px; height: 35px; border: none; color: white; background: #11e1c2; border-radius: 5px;">
                                <a href="{{route('reelsUpload')}}" class="xn" style="color:white;">Xác nhận</a>
                            </button>
                            <button id="noButton" style="width: 90px; height: 35px; border: none; color: black; background-color: #0000000e; border-radius: 5px;">
                                Trở về
                            </button>
                        </div>
                    </div>
    ...
    <script>
        window.onload = function() {
            ...
            if (videoURL) {
                ...
                if (videoSize) {
                    const sizeInMB = (videoSize / (1024 * 1024)).toFixed(2); // Chuyển đổi và làm tròn đến 2 chữ số thập phân
                ...
                if (videoDuration) {
                    ...
                    document.getElementById('videoDuration').querySelector('b').innerText = '0 seconds'; // Nếu là undefined thì mặc định là '0 seconds'
                }
            } else {
    ...
    <style>
        #videoFrame {
            ...
            overflow: hidden; /* Đảm bảo rằng các góc tròn không bị cắt */
        }
        textarea {
        ...
        .error-msg {
            position: fixed;
            top: -50px; /* Đặt thông báo ra ngoài màn hình */
            left: 50%;  
            transform: translateX(-50%);
            background-color: rgba(0, -1, 0, 0.65); /* Màu nền */
            color: white; /* Màu chữ */
            padding: 10px 20px; /* Khoảng cách bên trong */
            border-radius: 5px; /* Bo góc */
            z-index: 1000; /* Đảm bảo hiển thị trên các phần tử khác */
            transition: top 0.5s ease, opacity 0.5s; /* Hiệu ứng chuyển tiếp cho vị trí và độ mờ */
